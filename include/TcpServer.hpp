 ...
 * @LastEditTime: 2026-04-08 20:32:55
...
private:
    ...
    ///< 账本：记录当前餐厅里所有的客人。Key 是 fd，Value 是对应的 TcpConnection
    ///< 对象
    ...
    int nextConnId_; ///< 单调递增的连接 ID，用于唯一标识每个连接（防止 fd
                     ///< 复用导致误删）
...
public:
     ...
     * @details 内部会自动创建负责接收连接的 Acceptor
     * 迎宾员，并绑定新连接的调度回调(newConnection)。
     */
    TcpServer(EventLoop *loop, int port);

    /**
     * @brief 析构函数：释放 Acceptor
     * 并销毁账本(connections_)中所有尚未断开的客户端连接资源。
     */
    ~TcpServer();
     ...
     * @details
     * 核心职责是：在底层抛出销毁请求时，从账本(connections_)中将该连接擦除，并销毁该
     * TcpConnection 对象的堆内存。
     ...
     * @details 由 removeConnection 投递到主线程执行，保证 connections_ map
     * 只在主线程被访问
     */
     ...
     * @param cb 回调签名，提供对应的连接智能指针 `std::shared_ptr<TcpConnection>`
     * 以及解码出的纯文本信息 `const std::string&`
     */
