 ...
 * @Date: 2026-04-04 16:50:52
 * @LastEditors: Zhang YuHua 1774630667@qq.com
 * @LastEditTime: 2026-04-15 21:06:19
 * @FilePath: /OmniGateway/src/Logger.cpp
...
// 2. 辅助数组：把枚举转换成字符串前缀
// ==========================================================
const char *LogLevelName[4] = {"[INFO] ", "[WARN] ", "[ERROR]", "[FATAL]"};
...
Logger::Logger(const char *file, int line, LogLevel level)
  ...
  // 提示：std::this_thread::get_id() 返回的是个对象，你可以把它转成简单的 hash
  // 整数或者直接忽略，为了简单，先追加一个常量字符串如 "[TID] " 占位。
  // 为了简单和高性能，暂时不格式化 std::thread::id，直接用占位符
  stream_ << "[TID] "; // 实际项目中可以缓存哈希值
}

Logger::~Logger() {
  // 1. 日志的正文其实在构造到析构之间，已经被业务层通过 << 追加到 stream_
  // 里了！
  // 2. 打印文件名和行号，并加上换行符！
  stream_ << " - " << file_ << ':' << line_ << '\n';
