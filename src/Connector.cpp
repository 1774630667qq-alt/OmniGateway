 ...
 * @LastEditTime: 2026-04-07 12:02:54
...
void Connector::connect() {
     ...
     *        [附加标志位 (Linux 扩展)]
     *        - SOCK_NONBLOCK: 将套接字设为非阻塞模式。调用 accept/connect/读写 时不会阻塞当前线程
     *        - SOCK_CLOEXEC: 设置 close-on-exec 标志。在子进程中执行 exec() 时自动关闭该套接字，防止 fd 泄漏
     * @param protocol 指定特定的协议代码。通常设为 0，表示使用由 domain 和 type 组合决定的默认协议 (例如 TCP)
     * @return 成功时返回新创建的套接字文件描述符 (非负整数)；失败时返回 -1 (并设置 errno)
     */
     ...
     * @param addr 目标地址结构体指针。由于 connect 是通用的，需将特定的地址结构 (如 sockaddr_in) 
     *             强转为通用的 sockaddr* 类型。内核会根据 addrlen 和 addr 内的 sa_family 成员来解析具体地址。
     * @param addrlen 地址结构体的长度 (sizeof(struct sockaddr_in))
     * @return 
     *        - 0: 连接立即成功完成 (通常见于本地回环或已经就绪的情况)
     *        - -1: 出错，需检查 errno。在 **非阻塞模式 (SOCK_NONBLOCK)** 下，常见错误码：
     *          - [EINPROGRESS]: 连接正在异步建立中。这是非阻塞 connect 的正常预期行为，
     *                           后续需通过 poll/epoll 监听该 fd 的写事件来判断连接是否完成。
     *          - [EISCONN]: 套接字已经连接。
     *          - [ECONNREFUSED]: 目标地址拒绝连接。
     *          - [ETIMEDOUT]: 连接超时。
     *          - [EADDRNOTAVAIL]: 无法分配请求的地址。
     */
    int ret = ::connect(sockfd, (sockaddr*)serverAddr_.getSockAddr(), serverAddr_.getSockAddrLen());
    int savedErrno = (ret == -1) ? errno : 0;
    auto self = shared_from_this();
    switch (savedErrno) {
        // ────────── 连接正常或正在建立中，挂载 Channel 监听可写事件 ──────────
        case 0:            // connect() 立即成功 (常见于本地回环连接)
        case EINPROGRESS:  // TCP 三次握手正在异步进行中 —— 非阻塞 connect 的正常预期返回值
        ...
        // ────────── 不可恢复的致命错误，放弃连接 ──────────
        case EACCES:       // 权限不足：尝试连接广播地址但未设置 SO_BROADCAST，或受 SELinux/防火墙拦截
...
void Connector::retry(int sockfd, int savedErrno) {
    ...
    LOG_WARNING << "connect() 遇到可恢复错误 (errno=" << savedErrno
                << " " << strerror(savedErrno) << ")，将在 "
                << retryDelayMs_ << "ms 后重试";

    // 先用当前延迟调度重连，再翻倍 (指数退避，上限 30s)
    loop_->runAfter(retryDelayMs_, [self = shared_from_this()] () {
        self->connect();
    });
